...
export default function RegisterForm() {
  const [form, setForm] = useState({
    nik: "",
    phone: "",     // ✅ ubah dari telepon → phone
    email: "",
    name: "",      // ✅ ubah dari nama → name
    gender: "L", // ✅ ubah dari "L" → "Laki-laki"
    password: "",
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded shadow-md w-full max-w-sm">
      ...
      <input type="text" name="phone" placeholder="Telepon" className="w-full p-2 mb-4 border rounded" value={form.phone} onChange={handleChange} required />
      ...
      <input type="text" name="name" placeholder="Nama Lengkap" className="w-full p-2 mb-4 border rounded" value={form.name} onChange={handleChange} required />
      ...
      <button
        type="button"
        onClick={() => navigate('/')}
        className="w-full bg-transparent text-gray-600/50 py-3 mt-3 rounded-lg transition duration-300 flex items-center justify-center gap-2"
      >
        Kembali ke Beranda
      </button>
